...
FOCUS_BODY = r'''() => {
  const editor = document.querySelector('.FeEditorApp-_9ddb7e475b559749-editor, .FeEditorApp-_377c94a778c072b3-editor, #ueditor_0') || document.body;
  const rect = editor.getBoundingClientRect ? editor.getBoundingClientRect() : {x:200,y:200,width:600,height:300};
  const x = Math.max(40, rect.x + Math.min(80, rect.width / 4));
  const y = Math.max(140, rect.y + Math.min(120, rect.height / 4));
  if (editor.focus) { try { editor.focus({preventScroll:true}); } catch (_) {} }
  for (let i = 0; i < 2; i++) {
    document.body.dispatchEvent(new MouseEvent('mousedown', {bubbles:true,cancelable:true,view:window,clientX:x,clientY:y,button:0}));
    document.body.dispatchEvent(new MouseEvent('mouseup', {bubbles:true,cancelable:true,view:window,clientX:x,clientY:y,button:0}));
    document.body.dispatchEvent(new MouseEvent('click', {bubbles:true,cancelable:true,view:window,clientX:x,clientY:y,button:0}));
  }
  document.dispatchEvent(new KeyboardEvent('keydown', {key:'Escape', code:'Escape', keyCode:27, which:27, bubbles:true, cancelable:true}));
  return {x, y, cls: String(editor.className || ''), tag: editor.tagName || 'BODY'};
}'''
...
CLOSE_RIGHT = r'''() => {
  ...
  const rows = els.map(el => {
    ...
    const r = el.getBoundingClientRect();
    let score = 0;
    if (/收起|关闭|隐藏/.test(t)) score += 260;
    if (/close|collapse|fold/i.test(cls)) score += 180;
    if (r.x > 920 && r.y > 120 && r.y < 900) score += 80;
  ...
  const picked = rows[0];
  if (!picked) return {ok:false, rows: rows.slice(0,10)};
  const r = picked.rect;
  if (typeof picked.el.click === 'function') picked.el.click();
  picked.el.dispatchEvent(new MouseEvent('click', {bubbles:true,cancelable:true,view:window,clientX:r.x+r.w/2,clientY:r.y+r.h/2,button:0}));
...
CLICK_PUBLISH = r'''() => {
  ...
  const picked = rows[0];
  if (!picked) return {ok:false, rows: rows.slice(0,20)};
  const r = picked.rect;
  if (typeof picked.el.click === 'function') picked.el.click();
  for (const type of ['pointerdown','mousedown','pointerup','mouseup','click']) {
    picked.el.dispatchEvent(new MouseEvent(type,{bubbles:true,cancelable:true,view:window,clientX:r.x+r.w/2,clientY:r.y+r.h/2,button:0}));
...
async def main():
    outdir = base / 'debug' / 'publish_focus_body_then_publish'
    ...
    context = await pw.chromium.launch_persistent_context(
        str(base / f'edge_profile_publish_focus_{int(time.time())}'),
        channel='msedge',
        headless=False,
    ...
    result['before_focus'] = await page.evaluate(READ)
    result['focus_body'] = await page.evaluate(FOCUS_BODY)
    await page.wait_for_timeout(1200)
    result['close_right'] = await page.evaluate(CLOSE_RIGHT)
    await page.wait_for_timeout(1200)
    result['after_cleanup'] = await page.evaluate(READ)
