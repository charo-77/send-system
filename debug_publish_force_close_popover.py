...
FORCE_CLOSE_POPOVER = r'''() => {
  ...
  const popovers = Array.from(document.querySelectorAll('.cheetah-popover, [class*="popover"], .FeEditorApp-_4ecaee52b311664f-pop'));
  const removed = [];
  for (const el of popovers) {
    const txt = (el.innerText || '').slice(0, 120);
    const cls = String(el.className || '');
    const r = el.getBoundingClientRect();
    removed.push({txt, cls, rect:{x:r.x,y:r.y,w:r.width,h:r.height}});
    try { el.remove(); } catch (_) { try { el.style.display = 'none'; el.style.pointerEvents = 'none'; el.style.visibility = 'hidden'; } catch (_) {} }
  }
  document.body.style.overflow = 'auto';
  document.body.style.pointerEvents = 'auto';
  return {
    entryOpen: !!document.querySelector('.FeEditorApp-_4ecaee52b311664f-entry.cheetah-popover-open'),
    removed,
    remaining: Array.from(document.querySelectorAll('.cheetah-popover, [class*="popover"], .FeEditorApp-_4ecaee52b311664f-pop')).length
  };
}'''
...
READ = r'''() => ({
  ...
  popovers: Array.from(document.querySelectorAll('.cheetah-popover, [class*="popover"], .FeEditorApp-_4ecaee52b311664f-pop')).map((el, i) => {
    const r = el.getBoundingClientRect();
...
async def main():
    outdir = base / 'debug' / 'publish_force_close_popover'
    ...
    context = await pw.chromium.launch_persistent_context(
        str(base / f'edge_profile_publish_force_{int(time.time())}'),
        channel='msedge',
        headless=False,
    ...
    result['before_force_close'] = await page.evaluate(READ)
    result['force_close_popover'] = await page.evaluate(FORCE_CLOSE_POPOVER)
    await page.wait_for_timeout(1000)
    result['after_force_close'] = await page.evaluate(READ)
