...
INIT = r'''() => {
  if (window.__pubProbeInstalled) return true;
  window.__pubProbeInstalled = true;
  window.__pubProbeLog = [];
  const log = (type, data) => { try { window.__pubProbeLog.push({t:Date.now(), type, data}); } catch (_) {} };
  const desc = (el) => {
    if (!el) return null;
    ...
    return {
      ...
      txt: (el.innerText || el.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 200),
  ...
  const mo = new MutationObserver((mutations) => {
    for (const m of mutations) {
      for (const node of m.addedNodes || []) {
        ...
        all.forEach(el => {
          const blob = [el.innerText || '', el.className || '', el.id || '', el.outerHTML || ''].join(' ');
          if (/发布|确认|继续|提示|错误|封面|摘要|分类|标签|知道了|仍要发布|dialog|modal|drawer|popup|toast/i.test(blob)) {
            log('interesting', desc(el));
          }
        });
      }
    }
...
PREPARE_COVER_AND_CLEAN = r'''() => {
  ...
  clickBest((t, aria) => (t === '选择封面' ? 320 : 0) + (t === '更换' ? 220 : 0) + (/更换封面/.test(aria) ? 260 : 0));
  clickBest((t, aria, el) => (t === '三图' ? 300 : 0) + (t.includes('三图') ? 150 : 0) + (el.tagName === 'INPUT' && el.getAttribute('type') === 'radio' && el.getAttribute('value') === 'three' ? 250 : 0));
  ...
  if (entry) entry.classList.remove('cheetah-popover-open');
  for (const el of Array.from(document.querySelectorAll('.cheetah-popover, [class*="popover"], .FeEditorApp-_4ecaee52b311664f-pop'))) {
  ...
  return {
    ...
    popovers: Array.from(document.querySelectorAll('.cheetah-popover, [class*="popover"], .FeEditorApp-_4ecaee52b311664f-pop')).length,
  };
}'''
...
ACT_ON_INTERCEPT = r'''() => {
  const candidates = Array.from(document.querySelectorAll('button,[role=button],div,span,a')).map(el => {
    const t = (el.innerText || el.textContent || '').trim();
    const cls = String(el.className || '');
    const r = el.getBoundingClientRect();
    let score = 0;
    if (/继续发布|确认发布|确认|继续|仍要发布|知道了|我知道了/.test(t)) score += 400;
    if (/确定/.test(t) && !/存草稿/.test(t)) score += 280;
    if (cls.includes('primary')) score += 80;
    return {el, t, cls, score, rect:{x:r.x,y:r.y,w:r.width,h:r.height}};
  }).filter(x => x.score > 0 && x.rect.w > 20 && x.rect.h > 20).sort((a,b) => b.score - a.score || b.rect.y - a.rect.y);
  const actions = [];
  for (const picked of candidates.slice(0, 3)) {
    const r = picked.rect;
    if (typeof picked.el.click === 'function') picked.el.click();
    for (const type of ['pointerdown','mousedown','pointerup','mouseup','click']) {
      picked.el.dispatchEvent(new MouseEvent(type,{bubbles:true,cancelable:true,view:window,clientX:r.x+r.w/2,clientY:r.y+r.h/2,button:0}));
    }
    actions.push({t:picked.t, cls:picked.cls, rect:picked.rect});
  }
  return actions;
}'''
...
READ = r'''() => ({
  ...
  dialogs: Array.from(document.querySelectorAll('[role=dialog], [class*="dialog"], [class*="modal"], [class*="drawer"], [class*="popup"], [class*="popover"]')).map((el, i) => {
    const r = el.getBoundingClientRect();
    return {i, cls:String(el.className||'').slice(0,140), txt:(el.innerText||'').slice(0,500), rect:{x:r.x,y:r.y,w:r.width,h:r.height}};
  }).filter(x => x.rect.w > 40 && x.rect.h > 20).slice(0, 50),
  interesting: Array.from(document.querySelectorAll('*')).map(el => {
    const t = (el.innerText || el.textContent || '').trim();
    const r = el.getBoundingClientRect();
    return {t:t.slice(0,200), cls:String(el.className||''), rect:{x:r.x,y:r.y,w:r.width,h:r.height}};
  }).filter(x => x.t && x.rect.w > 20 && x.rect.h > 20 && /发布|确认|继续|提示|错误|封面|摘要|分类|标签|知道了|仍要发布|成功|草稿/.test(x.t)).slice(0, 100),
  probeLog: (window.__pubProbeLog || []).slice(-200)
})'''

async def main():
    outdir = base / 'debug' / 'publish_intercept_probe'
    ...
    context = await pw.chromium.launch_persistent_context(
        str(base / f'edge_profile_publish_intercept_{int(time.time())}'),
        channel='msedge',
        headless=False,
    ...
    result['prepare'] = await page.evaluate(PREPARE_COVER_AND_CLEAN)
    await page.wait_for_timeout(1200)
    result['before_publish'] = await page.evaluate(READ)
    result['click_publish'] = await page.evaluate(CLICK_PUBLISH)
    await page.wait_for_timeout(2000)
    result['after_publish_2s'] = await page.evaluate(READ)
    result['act_on_intercept'] = await page.evaluate(ACT_ON_INTERCEPT)
    await page.wait_for_timeout(4000)
    result['after_intercept_action'] = await page.evaluate(READ)

    (outdir / 'result.json').write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding='utf-8')
    await page.screenshot(path=str(outdir / 'after_intercept.png'), full_page=True)
