...
PREPARE_COVER = r'''() => {
  ...
  return {openCover, three, checkedBefore: checked.map(x => x.i), clicked, after};
}'''

CONFIRM_NEAR_COVER = r'''() => {
  ...
  const rows = all.map(el => {
    ...
    const r = el.getBoundingClientRect();
    let score = 0;
    if (/^确定(\s*\(\d+\))?$/.test(t)) score += 300;
    if (/完成|保存|应用/.test(t)) score += 220;
    if (cls.includes('primary')) score += 80;
    if (r.y > 2400) score += 120;
    if (r.x > 200 && r.x < 1200) score += 40;
    return {el, t, cls, score, rect:{x:r.x,y:r.y,w:r.width,h:r.height}};
  }).filter(x => x.score > 0 && x.rect.w > 10 && x.rect.h > 10).sort((a,b) => b.score - a.score || b.rect.y - a.rect.y);
  const picked = rows[0];
  if (!picked) return {ok:false, rows: rows.slice(0,40).map(x => ({t:x.t, cls:x.cls, score:x.score, rect:x.rect}))};
  const r = picked.rect;
  if (typeof picked.el.click === 'function') picked.el.click();
  for (const type of ['pointerdown','mousedown','pointerup','mouseup','click']) {
    picked.el.dispatchEvent(new MouseEvent(type,{bubbles:true,cancelable:true,view:window,clientX:r.x+r.w/2,clientY:r.y+r.h/2,button:0}));
  }
  return {ok:true, picked:{t:picked.t, cls:picked.cls, score:picked.score, rect:picked.rect}, rows: rows.slice(0,20).map(x => ({t:x.t, cls:x.cls, score:x.score, rect:x.rect}))};
}'''
...
async def main():
    outdir = base / 'debug' / 'cover_confirm_only'
    ...
    async with async_playwright() as p:
        context = await p.chromium.launch_persistent_context(
            str(base / f'edge_profile_cover_confirm_{int(time.time())}'),
            channel='msedge',
            headless=False,
        ...
        result['confirm_only'] = await page.evaluate(CONFIRM_NEAR_COVER)
