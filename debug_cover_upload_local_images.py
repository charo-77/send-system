...
OPEN_COVER = r'''() => {
  ...
  const rows = els.map(el => {
    ...
    if (/设置封面/.test(t)) score += 80;
    if (r.y > 2400) score += 80;
  ...
  return {ok:true, picked:{t:picked.t, cls:picked.cls, aria:picked.aria, score:picked.score, rect:picked.rect}};
}'''
...
SELECT_MODE = r'''(mode) => {
  ...
  const picked = rows[0];
  if (!picked) return {ok:false, target, rows: rows.map(x => ({t:x.t, cls:x.cls, score:x.score, rect:x.rect}))};
  const r = picked.rect;
  if (typeof picked.el.click === 'function') picked.el.click();
  for (const type of ['pointerdown','mousedown','pointerup','mouseup','click']) {
    picked.el.dispatchEvent(new MouseEvent(type,{bubbles:true,cancelable:true,view:window,clientX:r.x+r.w/2,clientY:r.y+r.h/2,button:0}));
  }
  return {ok:true, target, picked:{t:picked.t, cls:picked.cls, score:picked.score, rect:picked.rect}};
}'''
...
CHOOSE_AI = r'''() => {
  ...
  const rows = els.map(el => {
    ...
    const r = el.getBoundingClientRect();
    let score = 0;
    if (/AI.*封面|生成.*封面|智能.*封面/.test(t)) score += 300;
    return {el, t, score, rect:{x:r.x,y:r.y,w:r.width,h:r.height}, cls:String(el.className||'')};
  }).filter(x => x.score > 0 && x.rect.w > 10 && x.rect.h > 10).sort((a,b) => b.score - a.score);
  const picked = rows[0];
...
async def main():
    outdir = base / 'debug' / 'cover_upload_local_images'
    ...
    docx = files[0]
    extracted = extract_docx_images(docx, outdir / 'covers')
    sized = sorted([p for p in extracted if p.exists()], key=lambda p: p.stat().st_size, reverse=True)
    large = [str(p) for p in sized[:3]]
    one = [str(sized[0])] if sized else []
    tiny = [str(p) for p in sorted([p for p in extracted if p.exists()], key=lambda p: p.stat().st_size)[:1]]
    mode = 'three' if len(sized) >= 3 else 'one' if len(sized) >= 1 else 'ai'

    async with async_playwright() as p:
        context = await p.chromium.launch_persistent_context(
            str(base / f'edge_profile_cover_local_{int(time.time())}'),
            channel='msedge',
            headless=False,
        ...
        result = {
            'docx': str(docx),
            'mode': mode,
            'sorted_images': [{'path': str(p), 'size': p.stat().st_size} for p in sized],
            'selected_large': large,
            'selected_one': one,
            'selected_tiny': tiny,
        }
        ...
        if mode == 'ai':
            result['choose_ai'] = await page.evaluate(CHOOSE_AI)
        else:
            result['select_mode'] = await page.evaluate(SELECT_MODE, mode)
            await page.wait_for_timeout(1000)
            upload_targets = large if mode == 'three' else one
            upload_input = page.locator('input[type="file"][accept*="image" i], input[type="file"][multiple]').last
            await upload_input.set_input_files(upload_targets)
            result['cover_upload'] = {'ok': True, 'files': upload_targets}
            await page.wait_for_timeout(5000)
        ...
        print(json.dumps({
            ...
            'select_mode': result.get('select_mode'),
            'cover_upload': result.get('cover_upload'),
