...
ESCAPE_AND_CLEAN = r'''() => {
  const out = [];
  const fireEsc = target => {
    const e = new KeyboardEvent('keydown', {key:'Escape', code:'Escape', keyCode:27, which:27, bubbles:true, cancelable:true});
  ...
  if (body) {
    body.dispatchEvent(new MouseEvent('click', {bubbles:true,cancelable:true,view:window,clientX:10,clientY:10,button:0}));
    body.dispatchEvent(new MouseEvent('mousedown', {bubbles:true,cancelable:true,view:window,clientX:10,clientY:10,button:0}));
    body.dispatchEvent(new MouseEvent('mouseup', {bubbles:true,cancelable:true,view:window,clientX:10,clientY:10,button:0}));
  }
  const focusTargets = [
  ...
  for (const el of focusTargets) {
    try { el.focus({preventScroll:true}); out.push({focused: el.tagName, cls: String(el.className || '')}); } catch (_) {}
  }
  return out;
}'''
...
async def main():
    outdir = base / 'debug' / 'publish_escape_cleanup'
    ...
    context = await pw.chromium.launch_persistent_context(
        str(base / f'edge_profile_publish_escape_{int(time.time())}'),
        channel='msedge',
        headless=False,
    ...
    await page.wait_for_timeout(1200)
    result['before_escape'] = await page.evaluate(READ)
    result['escape_cleanup'] = await page.evaluate(ESCAPE_AND_CLEAN)
    await page.wait_for_timeout(1500)
    result['after_escape'] = await page.evaluate(READ)
