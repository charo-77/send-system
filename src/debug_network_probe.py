...
async def main() -> int:
    ...
    async with async_playwright() as p:
        ...
        async def on_response(resp):
            url = resp.url
            if not any(key in url.lower() for key in ["user", "author", "account", "member", "income", "profile", "quota"]):
                return
            try:
        ...
        (out / "probe_network.json").write_text(json.dumps(records, ensure_ascii=False, indent=2), encoding="utf-8")
        await browser.close()
    return 0
