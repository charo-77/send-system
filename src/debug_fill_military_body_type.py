...
async def main() -> None:
    outdir = base / "debug" / "fill_military_body_type"
    ...
    async with async_playwright() as p:
        context = await p.chromium.launch_persistent_context(
            str(base / f"edge_profile_fill_body_type_{int(time.time())}"),
            channel="msedge",
            headless=False,
        ...
        await page.screenshot(path=str(outdir / 'after_type.png'), full_page=True)
        data = {"meta": meta, "body_len": len(body_text), "count_text": count_text, "info": info, "url": page.url}
